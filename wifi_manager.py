...
class WifiManager:
    ...
    def __init__(self, ssid='IoTDevice', password='00000000', mode=0, reboot=True, debug=False):
        ...
        while True:
            ...
            try:
                ...
                if self.request:
                    ...
                    url = re.search('(?:GET|POST) /(.*?)(?:\\?.*?)? HTTP', self.request).group(1).decode(
                        'utf-8').rstrip('/')
                    if url == '':
                        self.handle_root()
                    elif url == 'configure':
